...
def index(request, pIndex=1):
    ...
    context = {"booklist": list2, 'plist': plist, 'pIndex': pIndex,
               'maxpages': maxpages, 'mywhere': mywhere, 'memberid': memberid}

    return render(request, "web/book/index.html", context)
...
def add(request):
    ...
    return render(request, "web/book/add.html", context)
...
def borrow(request):
    try:
        ...
        ob.create_at = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        ob.update_at = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        if (Book.objects.get(id=ob.book_id).number <= 0):
            context = {'info': "余量不足！借阅失败！"}
            return render(request, "web/info.html", context)
        ob.save()
        context = {'info': "申请借阅成功！"}
    except Exception as err:
