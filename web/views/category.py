...
def index(request, pIndex=1):
    ...
    # 执行分页处理
    pIndex = int(pIndex)
    page = Paginator(ulist, 12)  # 以每页8个数据分页
    ...
    return render(request, "web/category/index.html", context)
...
def add(request):
    ...
    return render(request, "web/category/add.html",context)
...
def edit(request, cid=0):
    try:
        ...
        return render(request, "web/category/edit.html", context)
    except Exception as err:
        print(err)
        context = {'info': "没有找到要修改的信息！！"}
        render(request, "web/category/edit.html", context)
