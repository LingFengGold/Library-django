...
def index(request, pIndex=1):
    ...
    # 执行分页处理
    pIndex = int(pIndex)
    page = Paginator(slist,8)  # 以每页5个数据分页
    ...
    return render(request, "web/shop/index.html", context)


def add(request):
    return render(request, "web/shop/add.html")
...
def insert(request):
    try:
        ...
        if not myfile:
            return HttpResponse("没有图书馆logo上传文件信息")
